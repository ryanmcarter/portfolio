...
// portfolio version is deliberately presentational because these labels do
// not navigate to case-study sections.
export function KraidleTabsShowcase() {
  ...
  const activeRect = rects[ACTIVE_INDEX];
  let growLeft = 0;
  let growWidth = 0;
        ...
        <div className="overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          >
            ...
            {tabs.map((tab, index) => (
              <span
                className={cn(
                  "relative z-10 inline-flex items-center justify-center whitespace-nowrap rounded border border-transparent px-4 py-3 text-sm font-medium leading-5 text-neutral-400 transition-colors sm:px-5 sm:text-base",
                  index === ACTIVE_INDEX && "text-neutral-50",
                )}
                data-kraidle-tab=""
                key={tab}
